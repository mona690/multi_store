...
class _ProductUploadPageState extends State<ProductUploadPage> {
  ...
  final String apiKey = 'gpBcy5ajmvkrzFdvrgFbAFSJ';
  FilePickerResult? result;

  bool isUploadingToCustomer = true; // Default option
  ...
  uploadData() async {
    ...
    if (imagesUrl.isNotEmpty) {
      ...
      DocumentSnapshot userDoc = await _firestore
          ...
          .get();
      final collection = isUploadingToCustomer ? 'products' : 'vendorsproducts'; // Choose collection based on radio button
      await _firestore.collection(collection).doc(productId).set({
        'productId': productId,
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text('تمت إزالة الخلفية بنجاح')));
              ...
              child: Column(
                ...
                children: [
                  ...
                  buildInputField(
                    ...
                    (value) {
                      if (value!.isEmpty) {
                        return "الرجاء إدخال سعر المنتج بعد الخصم";
                      } else {
                        return null;
                      }
                    },
                  ...
                  SizedBox(height: 16),
                  buildInputField('وصف المنتج', TextInputType.multiline, (value) {
                    description = value;
                  }, (value) {
                    if (value!.isEmpty) {
                  ...
                  Row(
                    ...
                    children: [
                      Radio(
                        value: true,
                        groupValue: isUploadingToCustomer,
                        onChanged: (value) {
                          setState(() {
                            isUploadingToCustomer = value as bool;
                          });
                        },
                      ),
                      Text('رفع المنتج إلى العميل'),
                      Radio(
                        value: false,
                        groupValue: isUploadingToCustomer,
                        onChanged: (value) {
                  ...
                  ButtonWidgets(
                    buttonChange: () async {
                      if (_formKey.currentState!.validate()) {
                        ...
                        ScaffoldMessenger.of(context).showSnackBar(
                            SnackBar(content: Text('يجب ادخال جميع البيانات')));
                      }
                    },
                    isLoading: _isUploading ? true : false,
                    buttonTitle: 'رفع المنتج',
                  ),
                ],
              ),
            ),
